...
def map_filename(obj, filename):
    del filename
    return "uploads/maps/{0}.map".format(obj.name)
...
class Map(models.Model):
    ...
    video = models.URLField(
        blank=True,
        default="",
        help_text="Must be a youtube link "
    ...
    def get_map_type(self):
        return self.map_types.all()[
            0
        ].id  # todo: change this! | just for being compatible with old teerace servers!
...
class Run(models.Model):
    ...
    server = models.ForeignKey(
        "Server", blank=True, null=True, related_name="runs", on_delete=models.SET_NULL
    )
    ...
    time = models.DecimalField(max_digits=MAX_DIGITS, decimal_places=DECIMAL_PLACES)
    ...
    def __str__(self):
        return "{0} - {1} - {2:.{precision}f}s".format(
...
class BestRun(models.Model):
    ...
    time = models.DecimalField(
        max_digits=Run.MAX_DIGITS, decimal_places=Run.DECIMAL_PLACES
    )
    ...
    def save(self, *args, **kwargs):
        ...
        if was_run_id != new_run_id:
            if self.run == self.map.best_score:
                action.send(self.user, verb="beat the record on", target=self.map)
            else:
                action.send(
                    self.user,
                    verb="beat {0} best score on".format(
                        "her" if self.user.profile.is_female() else "his"
                    ),
                    target=self.map,
                )
...
class Server(models.Model):
    ...
    description = models.TextField(blank=True, help_text="You may use Markdown syntax")
    ...
    played_map = models.ForeignKey(
        Map, null=True, blank=True, on_delete=models.SET_NULL
    )
