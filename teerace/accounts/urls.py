...
urlpatterns = [
    ...
    url(
        r"^profile/(?P<user_id>\d+)/points_graph.json$",
        profile_points_graph_json,
        name="profile_points_graph_json",
    ),
    url(r"^profile/(?P<user_id>\d+)/badges/$", profile_badges, name="profile_badges"),
    url(r"^profile/(?P<user_id>\d+)/best/$", profile_best, name="profile_best"),
    url(
        r"^profile/(?P<user_id>\d+)/activity/$",
        profile_activity,
    ...
    url(
        r"^password_reset/confirm/(?P<uidb64>[0-9A-Za-z_\-]+)/(?P<token>.+)/$",
        password_reset_confirm,
