...
def profile_points_graph_json(request, user_id):
    ...
    if user.profile.points_history:
        history = user.profile.points_history + [
            (user.profile.points_history[-1][0] + timedelta(1), user.profile.points)
        ]
    else:
        history = None

    response_data = simplejson.dumps(
        {"user": user.id, "history": history}, cls=DjangoJSONEncoder
    )
...
def profile_badges(request, user_id):
    ...
    return {"profile_user": user, "badges": badges}
...
def profile_best(request, user_id):
    ...
    return {"profile_user": user, "best_runs": best_runs}
...
def api_token(request):
    ...
    if request.method == "POST":
        ...
        messages.success(
            request,
            "New API token has been generated."
            " Please update your game configuration file.",
        )
    messages.info(
        request,
        "API token is required to log in the game." " You have to keep it secret!",
    )
    messages.warning(
        request,
        "<b>Make sure you want to do this!</b>"
...
def settings(request):
    ...
    if request.method == "POST":
        ...
        settings_profile_form = SettingsProfileForm(
        ...
        if settings_user_form.is_valid() and settings_profile_form.is_valid():
            ...
            messages.success(
                request, "Successfully updated your profile" " and settings."
            )
    return {
