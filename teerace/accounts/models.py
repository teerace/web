...
class UserProfile(models.Model):
    ...
    last_played_server = models.ForeignKey(
        "race.Server",
        blank=True,
        null=True,
    ...
    skin_body_color_raw = models.IntegerField(blank=True, null=True)
    skin_feet_color_raw = models.IntegerField(blank=True, null=True)
    ...
    @property
    def runtime(self):
        return Run.objects.filter(user=self.user).aggregate(Sum("time"))["time__sum"]
    ...
    def is_online(self):
        return self.last_connection_at >= timezone.now() - timedelta(minutes=10)
    ...
    def get_skin(self):
        if not self.has_skin:
            return None
        return {
            # "url": static(f"images/skins/{self.skin_name}.png"),
    ...
    def __str__(self):
        ...
        return f"{self.user}'{possessive} profile"
...
def post_user_save(instance, **kwargs):
    if kwargs["created"]:
        profile = UserProfile(user=instance, api_token=generate_random_key())
        profile.save()
