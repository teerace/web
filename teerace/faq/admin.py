...
def update_status(modeladmin, request, queryset, status):
    ...
    for obj in queryset:
        ...
        log_message = ugettext_noop(
            "Changed status to '%s'." % obj.get_status_display()
        )
...
class TopicAdmin(FAQAdminBase):
    fieldsets = (
        (
            None,
            {"fields": ("title", "slug", "description", "status", "sites", "ordering")},
        ),
    )
    inlines = (QuestionInline,)
    list_display = ("title", "description", "status", "question_count", "ordering")
    list_filter = ("status", "sites", "modified", "created")
    prepopulated_fields = {"slug": ("title",)}
    search_fields = ("title", "description")

    def get_queryset(self, request):
        return super().get_queryset().annotate(_question_count=Count("questions"))
